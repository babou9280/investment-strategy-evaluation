...
Le 16 juillet 2026, Ayman a suspendu la cohorte Gate 1 : le prototype actuel est un début utile mais le modèle doit aller plus loin avant observation. La mission active devient la conception Cost Gate vNext. Le Cost Ledger v1 initial est validé sur `df3ff7fc`, run `#650`. La revue hostile a ensuite séparé snapshot, projection et domaine de taille. Les renforcements CL-32 à CL-38 lient aussi instrument, place et horizon ; la Cost Survival Surface v1 croise neuf scénarios coût × avantage par taille dans 32 scénarios de falsification. L'ensemble renforcé est techniquement exécuté sur le head fonctionnel exact `fbc0d545b3ceff69dca9ceda00bc4f30e92c98ec`, run `#654`, artefact et captures inspectés. Le protocole préenregistré reste intact ; le compteur reste `0/5`.
...
La branche séparée est `strategy/cost-gate-offline-critique`. Le protocole et ses seuils restent préenregistrés dans `docs/tasks/COST_GATE_OFFLINE_CRITIQUE.md`, mais aucune observation n'est lancée. Le ledger renforcé, la revue hostile et la surface descriptive taille × coût × avantage possèdent une preuve fonctionnelle distante exacte sur `fbc0d545`, run `#654`. La surface reste une sensibilité de nominal, sans quantité exécutable, impact, capacité d'avantage, capital temporel ou fréquence. La prochaine conception doit traiter ces limites par contrats distincts avant toute nouvelle interface. Le compteur reste **0 sur 5**. Aucune donnée externe, connexion, recommandation, exécution ou publication n'est autorisée.
