function renderLedger(result) {
  ...
  const rows = result.evaluations.filter((row) => {
    ...
    const text = `${row.trade.ticker} ${row.trade.strategy} ${row.trade.sector} ${row.reason} ${row.resultBasisLabel} ${row.resultProvenance} ${price.sourceKey || ""} ${price.sourceCurrency || ""} ${price.conversionStatus || ""}`.toLowerCase();
  ...
  byId("ledger-body").innerHTML = rows.length ? rows.map((row) => {
    ...
    const price = row.position.priceDiagnostics || {};
    const factor = Number.isFinite(price.conversionFactor) ? ` × ${number.format(price.conversionFactor)}` : "";
    const priceLabel = price.convertible
      ? `${number.format(price.sourceValue)} ${price.sourceCurrency}${factor} → ${fmtCurrency(price.unitPriceEur)}`
      : price.conversionStatus === "missing" ? "prix absent" : `devise ${price.sourceCurrency || "non précisée"} non convertible`;
    return `<tr>
      ...
      <td><strong>${fmtCurrency(row.position.notional)}</strong><br>${row.position.units ? `${number.format(row.position.units)} unités` : "fractionné"}<br><small>${escapeHtml(priceLabel)}</small></td>
  ...
  const chips = [
    ...
    `${audit.live} live`,
    `${audit.eurPrices} prix EUR`,
    `${audit.usdPrices} prix USD`,
    `${audit.unsupportedPriceCurrencies} devise prix non supportée`,
