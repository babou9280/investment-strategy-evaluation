...
| BS-042 | P0 | Cost Gate / temps | Snapshot valide au calcul mais obsolète au moment d'usage | Faux sentiment d'actualité | mitigated | Revue du head `be6aa09` : `compareSnapshots` gardait les anciens constats actifs après expiration ; correction et transition fraîche→stale testées localement, CI exact-head requise |
| BS-043 | P1 | Cost Gate / orchestration | Synthèse unique supprimant plusieurs constats matériels | Diagnostic incomplet | mitigated | Revue du head `be6aa09` : stale masquait un conflit de source ; constats désormais indépendants et test local, CI exact-head requise |
...
| BS-054 | P0 | Cost Gate / unités | `operation_scope` incohérent avec `side_count` mais synthèse favorable produite | Frictions sous- ou surévaluées | mitigated | Mapping strict achat simple = 1, aller-retour = 2 ; sortie seule non supportée ; régressions locales, CI exact-head requise |
| BS-055 | P1 | Cost Gate / preuve | Ordre arbitraire des holds, sources, contraintes ou exclusions changeant le `snapshot_id` ou les constats | Faux changement et audit instable | mitigated | Ensembles canoniquement triés avant hash et évaluation ; snapshot et constats invariants par permutation, changement matériel distinct testé localement, CI exact-head requise |
| BS-056 | P1 | Cost Gate / orchestration | Erreur de friction empêchant un constat de cash indépendant | Diagnostic incomplet | mitigated | Erreurs de cash isolées des erreurs antérieures ; revue Codex et régression locale, CI exact-head requise |
| BS-057 | P1 | Cost Gate / vocabulaire | Avantage absorbé résumé comme contrainte utilisateur dépassée sans contrainte saisie | Provenance de la limite fausse | mitigated | Nouvelle synthèse `edge_not_surviving_modelled_friction` ; `constraint_breach` réservé aux limites explicites ; régressions locales, CI exact-head requise |
