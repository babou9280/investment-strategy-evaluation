...
| BS-087 | P0 | Cost Gate / dénominateur | Le seuil utilise un `returnDenominator` libre, absent de `basisValues` ou d'un montant différent | Seuil exact sur une base fictive | validated | Réconciliation clé-montant-devise et scénario fixe seul CL-32 ; head `fbc0d545`, run `#654` |
| BS-088 | P1 | Cost Gate / dépendances | `dependencies` est accepté sans propagation, cycles ni sémantique de calcul | Composant calculé malgré une dépendance invalide | validated | Toute liste non vide est refusée en v1, CL-33 sur head `fbc0d545`, run `#654` ; une future sémantique devra être versionnée |
| BS-089 | P0 | Cost Gate / projection | Un snapshot de coût est extrapolé comme fonction de taille sans domaine | Courbe précise mais non justifiée | validated | Politique de projection et domaine synthétique séparés ; CSS-09 à CSS-15 sur head `fbc0d545`, run `#654` ; aucune calibration réelle revendiquée |
...
| BS-091 | P0 | Cost Gate / scénarios | Coûts et avantages bas/central/haut sont appariés seulement en diagonale | Six scénarios hostiles cachés | validated | Produit cartésien neuf cellules par taille, CSS-01/02 et invariant de cardinalité sur head `fbc0d545`, run `#654` |
...
| BS-096 | P1 | Cost Gate / frontière | Une frontière est interpolée entre tailles dont l'avantage ou les coûts changent librement | Seuil inventé et fausse monotonie | validated | Formule exacte seulement sous linéarité prouvée ; profil explicite sans interpolation, falsifié sur head `fbc0d545`, run `#654` |
| BS-097 | P0 | Cost Gate / égalité | L'arrondi flottant transforme l'égalité au seuil en marge positive | État économique faux à la frontière | validated | Tolérance relative versionnée, état `at_threshold_no_positive_margin`, CSS-03/04 et propriété dédiée sur head `fbc0d545`, run `#654` |
| BS-098 | P1 | Cost Gate / UX | Une grille ou frontière devient implicitement un optimiseur de taille | Conseil ou approbation perçue | validated | Aucun optimum, score, classement ou taille proposée dans le contrat et CSS-28 sur head `fbc0d545`, run `#654` ; aucune interface de surface |
...
| BS-100 | P0 | Cost Gate / alignement | Le ledger perd instrument, place et horizon puis est confronté à un avantage seulement auto-déclaré aligné | Coûts d'un scénario comparés à l'avantage d'un autre | validated | `scenarioContext` et hash dans le ledger ; évaluateur canonique d'alignement réexécuté par la surface ; correspondance stricte et CSS-20/31/32 sur head `fbc0d545`, run `#654` |
