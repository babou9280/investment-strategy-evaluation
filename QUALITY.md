...
Le Cost Ledger v1 respecte ce standard dans son domaine synthétique. Sa tranche initiale CL-01 à CL-31 est prouvée sur `df3ff7fc`, run `#650`. Les renforcements CL-32 à CL-38 — dénominateur réconcilié, dépendances non supportées refusées, inclusion edge cohérente et contexte économique hashé — réussissent avec les dix propriétés de parité, oracles, intégrité, syntaxe et non-régressions sur le head fonctionnel exact `fbc0d545b3ceff69dca9ceda00bc4f30e92c98ec`, run `#654`. Cette validation ne transforme pas les hypothèses de coût en données calibrées.

Toute surface multi-taille doit désormais respecter `COST_SURVIVAL_SURFACE_CONTRACT.md` : domaine explicite, projection séparée du snapshot, contexte instrument/place/horizon réconcilié, profil d'avantage constant qualifié ou fourni par taille, produit cartésien coût × avantage, égalité distincte d'une marge positive et aucune frontière interpolée sans géométrie prouvée. Les suites CSS-01 à CSS-32, leurs oracles et leurs propriétés réussissent sur le même head exact `fbc0d545`, run `#654`. Cette preuve reste descriptive, synthétique et `notional_only_not_executable`.
