(function (root, factory) {
  ...
  function parseLocalizedNumber(raw, options) {
    ...
    if (!/^[+-]?(?:\d+(?:\.\d*)?|\.\d+)$/.test(normalized)) {
      return { status: "invalid", value: null, message: "Entre un nombre valide." };
    }
  ...
  function normalizeScenario(raw) {
    const input = raw || {};
    const states = {
      capitalEur: parseLocalizedNumber(input.capitalEur, { required: false, positive: true }),
    ...
    const activityState = activityType
      ? { status: "valid", value: activityType, message: null }
      : { status: "invalid", value: null, message: "Choisis un type d’activité." };
    states.activityType = activityState;

    const blockingFields = [
      "orderAmountEur",
      "monthlyFrequency",
      "commissionPerSideEur",
      "fxPerConversionPercent",
      "spreadPercent",
      "slippagePercent",
      "activityType",
    ];
    ...
    for (const [key, state] of Object.entries(states)) {
      if (state.status === "invalid" || (blockingFields.includes(key) && state.status === "missing")) {
        errors[key] = state.message || "Valeur invalide.";
      }
    }
    ...
    return {
      ...
      errors: {},
      values: {
        capitalEur: states.capitalEur.status === "valid" ? states.capitalEur.value : null,
  ...
  function calculateNormalized(values, provenance) {
    ...
    const annualCostToCapitalRate = source.capitalEur && source.capitalEur > 0
      ? cleanZero(annualCostEur / source.capitalEur)
      : null;
    ...
    const components = COMPONENT_ORDER.map((key) => ({
      key,
      label: LABELS[key],
      inputValue: inputValues[key].value,
      inputUnit: inputValues[key].unit,
      status: "valid",
    ...
    return {
      ...
      inputs: Object.assign({}, source),
      sideCount,
      annualOperations,
      components,
      totalCostPerOperationEur,
      costRatePerOperation,
      annualCostEur,
      annualCostToCapitalRate,
      breakEvenGrossRatePerOperation: costRatePerOperation,
      hasAnyFriction: totalCostPerOperationEur > 0,
    };
  }
  ...
  function calculate(raw, options) {
    ...
    return {
      ...
      result: calculateNormalized(normalized.values, options && options.provenance),
    };
  }

  function buildComparisons(result) {
    const current = calculateNormalized(result.inputs, "user_assumption");
    const doubleOrder = calculateNormalized(Object.assign({}, result.inputs, {
      orderAmountEur: result.inputs.orderAmountEur * 2,
    }), "user_assumption");
    const halfFrequency = calculateNormalized(Object.assign({}, result.inputs, {
      monthlyFrequency: result.inputs.monthlyFrequency / 2,
    }), "user_assumption");
    return [
      { key: "current", label: "Scénario actuel", result: current },
      { key: "double_order", label: "Ordre ×2", result: doubleOrder },
      { key: "half_frequency", label: "Fréquence ÷2", result: halfFrequency },
    ];
  }

  function finiteDisplayValue(value) {
    return Number.isFinite(value) ? cleanZero(value) : null;
  }
  ...
  function buildSharePayload(result, options) {
    ...
    const payload = {
      ...
      activityType: result.inputs.activityType,
      orderAmountEur: result.inputs.orderAmountEur,
      monthlyFrequency: result.inputs.monthlyFrequency,
      commissionPerSideEur: result.inputs.commissionPerSideEur,
      fxPerConversionRate: result.inputs.fxPerConversionRate,
      spreadRate: result.inputs.spreadRate,
      slippageRate: result.inputs.slippageRate,
      totalCostPerOperationEur: result.totalCostPerOperationEur,
      costRatePerOperation: result.costRatePerOperation,
      annualCostEur: result.annualCostEur,
      annualCostToCapitalRate: includeCapital ? result.annualCostToCapitalRate : null,
    };
  ...
  function shareText(result, options) {
    ...
    if (options && options.includeCapital && payload.annualCostToCapitalRate !== null) {
      lines.splice(4, 0, `Impact annuel sur le capital indiqué : ${formatPercent(payload.annualCostToCapitalRate)}`);
    }
    return lines.join("\n");
  }
  ...
  const DEMO_SCENARIOS = {
    small_round_trip: {
      label: "Très petit capital",
      provenance: "synthetic_demo",
      values: { capitalEur: 500, orderAmountEur: 100, activityType: "round_trip", monthlyFrequency: 4, commissionPerSideEur: 1, fxPerConversionPercent: 0.25, spreadPercent: 0.10, slippagePercent: 0.10 },
    },
    intermediate_round_trip: {
      label: "Capital intermédiaire",
      provenance: "synthetic_demo",
      values: { capitalEur: 5000, orderAmountEur: 500, activityType: "round_trip", monthlyFrequency: 4, commissionPerSideEur: 1, fxPerConversionPercent: 0.25, spreadPercent: 0.10, slippagePercent: 0.10 },
    },
    monthly_eur_buy: {
      label: "Achat mensuel en euros",
      provenance: "synthetic_demo",
      values: { capitalEur: 5000, orderAmountEur: 300, activityType: "single_buy", monthlyFrequency: 1, commissionPerSideEur: 1, fxPerConversionPercent: 0, spreadPercent: 0.05, slippagePercent: 0.05 },
    },
    zero_commission_round_trip: {
      label: "Commission affichée à zéro",
      provenance: "synthetic_demo",
      values: { capitalEur: 10000, orderAmountEur: 1000, activityType: "round_trip", monthlyFrequency: 2, commissionPerSideEur: 0, fxPerConversionPercent: 0.25, spreadPercent: 0.10, slippagePercent: 0.10 },
    },
  };
