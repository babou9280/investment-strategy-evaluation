(function () {
  ...
  function rangeConstraintCard(edgeRange) {
    ...
    const p = document.createElement('p');
    p.textContent = 'Chaque valeur répond uniquement à l’hypothèse brute correspondante. Elle ne recommande aucune taille.';
    const dl = document.createElement('dl');
    dl.className = 'constraint-table';
    [['Basse', edgeRange.low], ['Centrale', edgeRange.base], ['Haute', edgeRange.high]].forEach(([name, hypothesis]) => {
      const dt = document.createElement('dt');
      dt.textContent = name;
      const dd = document.createElement('dd');
      const item = hypothesis.minimumOrderForPositiveNet;
      if (item.status === 'available') {
        dd.textContent = item.boundary === 'no_positive_minimum_from_fixed_costs' && item.value === 0
          ? 'Aucun minimum positif imposé par les coûts fixes'
          : `> ${fmtEur(item.value)}`;
      } else {
        dd.textContent = item.reason === 'structurally_unreachable'
          ? 'Impossible sous cette hypothèse'
          : 'Indisponible';
      }
      dl.append(dt, dd);
    });
    article.append(label, strong, p, dl);
    return article;
  }
  ...
  function renderRange(result) {
    ...
    rangeSection.hidden = result.edgeMode !== 'range_estimate';
    if (result.edgeMode !== 'range_estimate') return;

    setText('range-summary', rangeStateCopy(range.rangeState));
    [['low', 'low'], ['base', 'base'], ['high', 'high']].forEach(([idPart, key]) => {
      const hypothesis = range[key];
  ...
  function selectedConstraint(result) {
    ...
    if (mode === 'retention') {
      return result.edgeMode === 'range_estimate'
        ? constraintCard('Taille frontière pour conserver la part choisie', { status: 'unavailable', reason: 'range_mode_selected', value: null }, fmtEur, '')
