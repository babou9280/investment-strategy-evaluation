...
- Phase : Gate 1 Cost Gate, prototype initial requalifié en sonde technique ; Cost Ledger v1 isolé en cours de validation avant toute cohorte
...
Cette preuve technique produit des `findings[]`, réconcilie cash immédiat et coût du cycle, plafonne la faisabilité par l'allocation de stratégie et invalide les snapshots modifiés. Gate 1 l'expose dans un prototype `internal_review` hors ligne techniquement validé sur son head exact. Ce prototype est conservé comme sonde technique pendant la conception du Cost Ledger v1 ; cela ne constitue ni une compréhension utilisateur démontrée, ni un produit validé, ni un moteur de données de marché, ni une lecture de compte réel.
